...
import os
import argparse
from src.utils import nambu_diagonalization_ising_model, parallel_nambu_diagonalization_ising_model
...
parser.add_argument(
    "--train", type=bool, help="if True, prepare the train dataset (default=True)",action=argparse.BooleanOptionalAction
)
parser.add_argument(
...
for i,h in enumerate(h_max):
    ...
    hs,ms,_,f,fm,e=parallel_nambu_diagonalization_ising_model(nbatch=args.nbatch,l=l,j_coupling=j_coupling,hs=hs,device=args.device,pbc=pbc)
    if i==0:
        hs_tot=hs
        ms_tot=ms
        fm_tot=fm
...
if args.train:
    ...
    np.savez(f'data/dataset_1nn/train_'+args.file_name+text_z2+f'_{l}_l_'+text_field+f'_{ms_tot.shape[0]}_n',density=ms_tot,density_F=fm_tot,F=f_tot)
else:
    np.savez(f'data/dataset_1nn/valid_'+args.file_name+f'_{l}_l_'+text_field+f'_{ms_tot.shape[0]}_n',density=ms_tot,density_F=fm_tot,F=f_tot,potential=hs,energy=e_tot)
