...
def nambu_diagonalization_ising_model(l:int,h_max:float,j_coupling:float,checkpoint:bool,train:bool,device:str,name_file:str,pbc:bool,n_data:int):
    ...
    if checkpoint:
        if train:
            np.savez(f'data/dataset/train_sequential_'+name_file+f'_{l}_l_{h_max:.1f}_h_{n_dataset}_n.npz',energy=es.cpu().numpy(),F=fs.cpu().numpy(),density=m_zs.cpu().numpy(),potential=hs[0:m_zs.shape[0]].cpu().numpy())        
        else:
            np.savez(f'data/dataset/valid_sequential_'+name_file+f'_{l}_l_{h_max:.1f}_h_{n_dataset}_n.npz',energy=es.cpu().numpy(),F=fs.cpu().numpy(),density=m_zs.cpu().numpy(),potential=hs[0:m_zs.shape[0]].cpu().numpy())
...
def nambu_correlation_ising_model(l:int,j_coupling:float,hs:np.array,device:str,name_file:str,pbc:bool):
    """Compute the correlation <\sigma_x \sigma_x>(ij) of the transverse quantum ising model using the Nambu Mapping
    ...
    for i in trange(n_dataset):
        ...
        for k in range(l):
            for r in range(k,l):
                ...
                if r!=k:
                    ss_z[i,k,r]=ss_z[i,k,r]+(c[k,k]*c[r,r]-c[k,r]*c[r,k])
                    ss_z[i,r,k]=ss_z[i,r,k]+ss_z[i,k,r]
                    #print(f'k={k},j={j}')
                    ss_x[i,k,r]=ss_x[i,k,r]+torch.linalg.det(c[k:r,k+1:r+1])
...
def parallel_nambu_correlation_ising_model(nbatch,l:int,j_coupling:float,hs:np.array,device:str,name_file:str,pbc:bool):
    """Compute the correlation <\sigma_x \sigma_x>(ij) of the transverse quantum ising model using the Nambu Mapping
    ...
    for i in trange(nbatch):
        ...
        for k in range(l):
            for r in range(k,l):
                if r!=k:
                    ss_z[:,k,r]=ss_z[:,k,r]+(c[:,k,k]*c[:,r,r]-c[:,k,r]*c[:,r,k])
                    ss_z[:,r,k]=ss_z[:,r,k]+ss_z[:,k,r]
                    #print(f'k={k},j={j}')
                    ss_x[:,k,r]=ss_x[:,k,r]+torch.linalg.det(c[:,k:r,k+1:r+1])
...
# %%
def nambu_energy_gap_ising_model(l:int,h_max:float,j_coupling:float,device:str,pbc:bool,n_data:int):
    """Diagonalization of the transverse quantum ising model using the Nambu Mapping
...
def nambu_from_magnetization_to_energy_density(nbatch:int,l:int,j_coupling:float,hs:np.array,device:str,pbc:bool):
    """Compute the correlation <\sigma_x \sigma_x>(ij) of the transverse quantum ising model using the Nambu Mapping
